...
class FireMage(UnitCard):
    def on_play(self, player, opponent):
        """打出时对敌方所有派出的单位造成2点伤害"""
        battlefield = opponent.game.battlefields[opponent]
        backline_units = battlefield['backline']
        frontline_units = battlefield['frontline']
        all_opponent_units = backline_units + frontline_units
        
        
        if all_opponent_units:
            target = random.choice(all_opponent_units)
