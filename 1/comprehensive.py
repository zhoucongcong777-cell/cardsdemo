...
class UnitCard(Card):
    """单位卡牌"""
    def __init__(self, name, description, cost, attack, health, action):
...
class Player:
    ...
    def initialize_deck(self):
        """初始化玩家的卡组"""
        # 添加单位卡
        self.deck.append(UnitCard("勇敢的士兵", "普通士兵单位", 0, 2, 4, 1))
        self.deck.append(UnitCard("精锐弓箭手", "远程攻击单位", 1, 3, 2, 1))
        self.deck.append(UnitCard("治疗牧师", "打出时治疗玩家3点生命", 4, 1, 4, 2))
        self.deck.append(UnitCard("火焰法师", "打出时对敌方单位造成4点伤害", 5, 4, 3, 3))
        self.deck.append(UnitCard("重装骑士", "高生命值单位", 5, 4, 7, 4))
