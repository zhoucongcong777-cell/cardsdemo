...
class Player:
    ...
    def initialize_deck(self, default=None):
        """初始化玩家的卡组"""
        # 添加单位卡
        self.deck.append(UnitCard("勇敢的士兵", "普通士兵单位", 0, 3, 3))
        self.deck.append(UnitCard("精锐弓箭手", "远程攻击单位", 0, 3, 2))
        self.deck.append(UnitCard("治疗牧师", "打出时治疗玩家3点生命", 0, 1, 4))
        self.deck.append(UnitCard("火焰法师", "打出时对敌方单位造成2点伤害", 0, 4, 3))
        self.deck.append(UnitCard("重装骑士", "高生命值单位", 0, 4, 7))
        
        # 添加法术卡
        self.deck.append(MagicCard("火球术", "对敌方玩家造成5点伤害", 0))
        self.deck.append(MagicCard("治疗之触", "恢复5点生命值", 0))
        self.deck.append(MagicCard("闪电链", "对所有敌方单位造成2点伤害", 0))
        self.deck.append(MagicCard("力量祝福", "使一个友方单位+2/+2", 0))
        self.deck.append(MagicCard("召唤援军", "抽两张牌", 0))
    ...
    def play_card(self, card_index, opponent):
        ...
        if card.on_play(self, opponent):
            ...
            if card.type == CardType.UNIT:
                print(f"{self.name} 在战场派出了 {card.name}！", end="\n_______________________________________________________________________________________________________")
